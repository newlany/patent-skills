---
name: patent-application-correction-docx
description: Process Chinese patent application correction work from a 补正通知书 and the original application Word files (.docx, or .doc after normalization). Use when Codex must extract the required corrections from the notice, edit the affected claims/specification/abstract in Word format, and generate replacement-page .docx files such as 权利要求书替换页 and 说明书替换页 without disturbing the original formatting and layout. This skill coordinates the existing minimax-docx, patent-docx-math, and doc skills for patent-safe DOCX handling.
---
...
- Use `patent-docx-math` before editing when the file may contain formulas, tracked changes, numbering chains, tables, comments, drawings, or other patent-sensitive structure.
...
- Run `patent-docx-math/scripts/inspect_docx.py` on the notice and each editable application DOCX when structure risk is non-trivial.
