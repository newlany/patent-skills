---
name: patent-stage-invention-content
description: "Internal specification-section specialist under patent-cn-draft for drafting or rewriting the 发明内容 section of a Chinese patent application. Direct use is allowed only for standalone 技术方案、有益效果、优选方案、发明内容重写 requests after claims or a stable drafting baseline exists."
---
# CN Patent Invention Content

## New Architecture Role

`patent-cn-draft` is the preferred public drafting orchestrator. This skill is the internal `05_specification` invention-content specialist. Direct use is appropriate only for standalone section drafting/revision requests or older prompts that explicitly name this skill.

## Skill Position

- 类型：线性撰写说明书阶段 / invention-content stage。
- 中文入口：发明内容、技术方案、有益效果、优选方案、按权利要求重写发明内容。
- 输入：稳定权利要求书、技术方案重构报告、背景缺陷或参考案件写法。
- 输出：发明内容章节文本，包括技术方案、优选方案、有益效果。
- 支撑能力：可读取 `patent-entry-drafting`、路线专科和 `patent-stage-embodiments` 的阶段产物。
- 边界：没有权利要求或稳定方案时，不要凭空扩展保护范围。
...
- For material, formulation, preparation, foaming, dispersion, or compounding cases, explain the cause in physical process terms. Prefer concrete mechanisms such as phase compatibility, polarity difference, viscosity and shear history, particle migration, interface wetting, moisture reaction with isocyanate, gas generation, cell nucleation, heat or mass transfer, residence time, or additive distribution. Do not stop at abstract theory words such as `分散性差`, `相容性不好`, or `反应不充分` unless the physical reason is also written.
- For high-rebound or high-energy-return foamed polymer cases, explain the problem through the physical use and forming process. Useful mechanisms include cell-wall bending, stretching, local compression, chain reversible deformation, chain slippage, viscoelastic dissipation, internal friction or deformation heat, residual stress release, gas migration, cell coarsening, local collapse, and oxidative chain degradation. Do not merely say that `能量回归率不高` or `软硬匹配不好`; write why the input energy fails to return as macroscopic rebound.
...
- When the claimed subject is revised from an application-specific product to a generic material or method, synchronize the subject wording across the title, purpose paragraph, solution paragraph, effects, terminology, and later handoff notes. Replace application-locked terms such as `鞋用`, `中底`, `中底模具`, or `发泡中底坯体` with claim-aligned generic terms such as `聚氨酯发泡材料`, `成型模具`, or `发泡材料坯体`, unless the claim still requires that application limitation.
...
- In material-process cases, tie effects to material behavior or process behavior, not only to chemical names. For example, explain how a carrier enters a reaction system, how water affects bubble formation, how a pre-dispersion changes pigment distribution, or how a process window changes melt state before claiming color, density, rebound, or compression-set effects.
...
- If the user supplies a completed reference specification as a style model, imitate its paragraph rhythm, section order, restrained effect language, and formatting level, not its technical facts. Outside the claim-aligned solution paragraph, avoid stiff invented labels and slogan-like terms; write in plain specification prose.
...
- Check for stale application-scope terms after a subject change, especially `鞋用`, `中底`, `模具`, `坯体`, and product names in the title, abstract, field, background, invention content, embodiments, and drawing legends.
