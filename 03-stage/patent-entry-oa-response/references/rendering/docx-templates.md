...
Using an 意见陈述 template is mandatory when the task requires a deliverable `.docx` OA response.

Template priority:

1. User-provided template named in the current request.
2. Case-folder template or local workspace template.
3. Bundled skill template only when neither 1 nor 2 exists.

Known local workspace templates:
...
Bundled fallback template assets:
...
When producing a final `.docx`, use [$doc](../../../doc/SKILL.md) to edit a working copy of the selected template. Do not rebuild formatting from scratch and do not merely imitate the template. The generated DOCX must originate from the chosen template file, preserving its section settings, styles, numbering definitions, and paragraph skeleton unless a case-specific correction is necessary.
